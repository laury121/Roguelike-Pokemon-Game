...
void generateTerrain(char terrain[HEIGHT][WIDTH], Gate *gates, int x, int y) {
    ...
    //hard coding gates
    if (gates->north != -1) terrain[0][gates->north] = PATH;
    if (gates->south != -1) terrain[HEIGHT - 1][gates->south] = PATH;
    if (gates->west != -1) terrain[gates->west][0] = PATH;
    if (gates->east != -1) terrain[gates->east][WIDTH - 1] = PATH;
